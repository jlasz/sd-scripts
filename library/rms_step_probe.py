...
def validate_rms_probe_configuration(args: argparse.Namespace) -> bool:
    ...
    if args.target_total_rms is not None:
        raise ValueError("RMS probe step estimation cannot be combined with --target_total_rms")
    if args.deepspeed:
...
def build_rms_probe_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    # The probe is a local diagnostic run. Keep its final weights and full training
    # state, but avoid periodic outputs, external uploads, samples, and tracker runs.
    ...
    probe_args.track_weight_values = False
    probe_args.target_total_rms = None
